...
# ========== НАСТРОЙКИ ==========
PROXY = None
ADMIN_ID = 6997318168  # ЗАМЕНИ НА СВОЙ ТЕЛЕГРАМ ID
SEARCH_INTERVAL = 300

# ID каналов для подписки (только ID, без -100 для проверки)
REQUIRED_CHANNELS = [
    {"id": -1002839409663, "invite_link": "https://t.me/+wBwWF5bz649kM2E6", "username": "канал1"},
    {"id": -1003391124726, "invite_link": "https://t.me/+jQ4RsEzy6TFhY2I0", "username": "канал2"}
]
...
async def is_user_subscribed(user_id):
    ...
    for channel in REQUIRED_CHANNELS:
        try:
            # Пробуем проверить через get_permissions
            ...
            # Другая ошибка (например, бот не админ в канале)
            print(f"Ошибка проверки канала {channel['id']}: {e}")
            # Если бот не может проверить — пропускаем этот канал
            continue
    return True

def get_subscription_keyboard():
    """Клавиатура для подписки (без стилей у URL-кнопок)"""
    buttons = []
    for ch in REQUIRED_CHANNELS:
        # У URL-кнопок НЕТ параметра style
        buttons.append([Button.url(f"📢 Подписаться", ch["invite_link"])])
    buttons.append([Button.inline("✅ Проверить подписку", "check_sub", style="success")])
    return buttons

def get_main_menu_inline():
    """Главное меню с анимированными эмодзи"""
    return [
...
async def start_userbot_with_bot_code():
    ...
    try:
        ...
        await bot.send_message(admin_entity, "❌ **Требуется двухфакторная аутентификация.**", parse_mode="markdown")
        raise
    finally:
        waiting_for_code = False
        code_future = None
...
async def girl_details(event):
    ...
    buttons = [
        [Button.url("✍️ Написать", f"https://t.me/{username}")],  # У URL-кнопок нет style
